...
// 4/2019: John Allwine added load_stl and write_stl for read/writing binary STL files
...
namespace Simplify
{
	...
	void load_stl(const char* filename) {
          ...
          for(int i = 0; i < num_tris; i++) {
              ...
              readBytes = fread(&point1, 1, 12,f);
              readBytes = fread(&point2, 1, 12,f);
	...
	void write_stl(const char* filename) {
          ...
          char header[81] = {0};
          snprintf(header, 81, "Simplified mesh using stl_decimate");
          ...
          for(int i = 0; i < num_tris; i++) {
              Vertex v1 = vertices[triangles[i].v[0]];
              Vertex v2 = vertices[triangles[i].v[1]];
              Vertex v3 = vertices[triangles[i].v[2]];
